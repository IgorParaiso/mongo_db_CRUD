...
class Controller_Laboratorio:
    ...
    def atualizar_laboratorio(self) -> Laboratorio:
        ...
        if self.verifica_se_existe(mongo, id_lab):
            ...
            if choice == 1:
                ...
                mongo.write(f"update laboratorios set qtd_maquinas = '{nova_qtd_maquinas}' where id_lab = '{id_lab}'")
                ...
                mongo.write(f"update laboratorios set tipo_lab = '{novo_tipo_lab}' where id_lab = '{id_lab}'")
                df_lab = mongo.sqlToDataFrame(f"select id_lab, qtd_maquinas, tipo_lab from laboratorios where id_lab = '{id_lab}'")
                cliente_atualizado = Laboratorio(df_lab.id_lab.values[0], df_lab.qtd_maquinas.values[0], df_lab.tipo_lab.values[0])
                print(cliente_atualizado.to_string())
                return cliente_atualizado
        ...
        if self.verifica_se_existe(mongo, id_lab):
            ...
            if self.verifica_se_existe_agenda(mongo, id_lab):
                ...
                if confirmation.upper() == "S":
                    df_cliente = mongo.sqlToDataFrame(f"select id_lab, qtd_maquinas, tipo_lab from laboratorios where id_lab = '{id_lab}'")
                    mongo.write(f"delete from laboratorios where id_lab = '{id_lab}'")
            ...
            if confirmation.upper() == "S":
                df_cliente = mongo.sqlToDataFrame(f"select id_lab, qtd_maquinas, tipo_lab from laboratorios where id_lab = '{id_lab}'")
                mongo.write(f"delete from agenda where id_lab = '{id_lab}'")
                mongo.write(f"delete from laboratorios where id_lab = '{id_lab}'")
    ...
    def verifica_se_existe(self, mongo:MongoQueries, cpf:int=None) -> bool:
        ...
        df_cliente = mongo.sqlToDataFrame(f"select id_lab, qtd_maquinas from laboratorios where id_lab = '{cpf}'")
        return df_cliente.empty

    def verifica_se_existe_agenda(self, mongo:MongoQueries, id_agenda:int=None) -> bool:

        df_agenda = mongo.sqlToDataFrame(f"select id_agenda, id_cliente, id_lab from agenda where id_lab = '{id_agenda}'")
